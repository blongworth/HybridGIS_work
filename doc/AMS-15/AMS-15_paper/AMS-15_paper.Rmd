...
Reference and process blank materials were not pretreated or cleaned. Carbonate shell samples for the core comparison study were picked from a split core and cleaned by washing with distilled water and dried at 60 C. Shell samples were crushed and mixed, but not otherwise pretreated. Other? Sonication?

Standards were prepared by evacuating vials and then filling with a modern standard CO~2~ using the automated gas handling system. Standards were adjusted to the same size as other samples by removing CO~2~ and replacing with helium using a syringe. Process blank samples were prepared from powdered C-1 or TIRI-F, acidified in vials using the method above.
...
We applied a correction for the regional marine reservoir effect with a delta R of -216 and uncertainty 92 based on the oxcal model with 10 reservoir points. Samples with radiocarbon age \> 0 were calibrated using the Marine20 curve @heaton_marine20marine_2020. The NH1 curve from Hua et. al @hua_atmospheric_2013 was used for calibration of postmodern dates.
