...
Our initial work with the NEC source established optimal source parameters for consistent source performance. We used tank CO2 connected through a regulator to the ion source to measure source performance over a range of CO2 flows. Optimal flow for this source is 7.5uLs-1 CO2, but the source produces near optimal currents over a wide range of flows. Adding helium to the gas mixture produced similar currents to pure CO2 at the same CO2 flow rates.

Best currents for the source are around 10uA 12C- or roughly 1/10 of graphite currents. This is generally comparable with other hybrid sources REFS. Typical currents for this study were between 4-9uA and average current for all samples was `r curmean`. The source typically produces best current at around 5uLs-1 CO2. Literature values give 1.5uLs-1 as optimal, but we've never seen that, and pressures, capillaries and flows in at least one of these papers doesn't make sense.
...
Mean Fm of blanks: `r mean(blanks$norm_ratio) %>% format(digits = 3)`
SD of Fm of blanks: `r sd(blanks$norm_ratio) %>% format(digits = 3)`
...
![Figure 2](figures/fig1a_flow_current_time.svg)

![Figure 2b](figures/fig1b_current_flow.svg)

![Figure 4](figures/fig2_current_dependent_blank.svg)

![Figure 4b](figures/fig3_cur_indep_blank.svg)

![Figure 5](figures/fig5_consensus_samples.svg)

![Figure 6](figures/fig6_core_carbonates.svg)
