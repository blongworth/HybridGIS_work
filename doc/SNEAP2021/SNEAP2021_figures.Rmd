...
secs <- ggplot(dfc, aes(fm_consensus, fm_diff, color = Cur)) +
  geom_hline(yintercept = 0) +
...
secs_lbc <- ggplot(dfc, aes(fm_consensus, fmbc_diff, color = Cur)) +
  geom_hline(yintercept = 0) +
  geom_smooth(method = "lm", se = FALSE) +
...
### Blanks - current independent

```{r}
df %>% filter(fm_consensus < 0.1 &
              Cur > 2) %>% 
  ggplot(aes(Cur, mean)) +
