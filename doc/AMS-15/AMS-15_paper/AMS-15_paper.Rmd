...
One of the greatest contrasts of the open split approach with closed-source HGIS methods is changing CO~2~ flow and helium dilution ratio over the period of measurement due to the dilution of the sample by the displacement helium. We examined the effect of this variable carbon delivery and dilution by measuring ^12^C^-^ current evolution during helium displacement/dilution of vial CO~2~ (Figure 2b) and comparing this to the composition of the gas delivered to the source. We estimated gas composition (Figure 2a) using a simple mixing model:

$$C(t) = e^{-\frac{r}{V}t}$$

Where $C(t)$ is the fraction CO~2~, $r$ is the rate of helium displacement flow, and $V$ is the initial volume of the CO~2~-filled vial. Figure 2c shows the resulting relationship of sample ion current to CO~2~ flow rate. The most important feature of the current-flow relationship is the wide range of flows under which stable ion current is measured, which in turn produces relatively stable currents and ratios despite a changing carbon flow rate during measurement. Several studies have shown peak currents over a narrower range of flows @Fahrni2013; @Broek2022 at high helium dilution rates (95%), while Xu et. al @Xu2007 shows a somewhat broader peak with pure CO~2~. Broek et. al @Broek2022 shows peak broadening with decreasing dilution ratio. While lower rates of dilution decrease current slightly, the observed peak broadening allows stable measurement over the range of CO~2~ flows produced by the open split.
...
A suite of 15 carbonate macrofossils consisting of shell and coral fragments from a series of coastal marine cores @moser2021 were measured as graphite and as CO~2~ using the HGIS method to further test the method and serve to demonstrate a potential application. Sediment cores were collected from Baie des Baradères, Haiti during a scientific cruise in May 2016 lead by Dr. Jeffrey Donnelly (WHOI). Coring was performed with a Rossfelder vibracorer from a 20-foot pontoon boat. The cores were collected from a coastal karst basin (CKB) at vertical depth \~11.7 m and coordinates 18.514° N, 73.629° W. Core HATC1 D1 is 145 cm long and was collected in a 3-inch poly tube as a control directly beside core HATC1 D2, which is a continuous 896 cm core collected in a 3-inch aluminum tube. Carbonate materials such as corals and bivalve shells were selected throughout the core for the age-depth model.

We found the HGIS and graphite measurements to be in good agreement, with no significant bias over the range in F^14^C of the core samples (0.4 - 1.15) (Figure 5a). Mean difference between hybrid and graphite measurements was -0.003 F^14^C and the standard deviation of this difference was 0.009 F^14^C. Mean per-sample 1σ uncertainty was 0.010 F^14^C for HGIS measurements, compared with 0.002 F^14^C for graphite. There is an apparent, but not significant positive relationship between the HGIS-graphite difference and measured value. The higher graphite values at lower F^14^C may indicate an excessive HGIS blank correction for these samples or an undercorrected method blank for graphite measurements.
...
We applied a correction for the regional marine reservoir effect with a delta R of -216 and uncertainty 92 based on the oxcal model with 10 reservoir points. Samples with pre-bomb radiocarbon ages were calibrated using the Marine20 curve @heaton_marine20marine_2020. The NH1 curve from Hua et. al @hua_atmospheric_2013 was used for calibration of postmodern dates, but these measurements were not included in the age model due to uncertainties in reservoir correction.
