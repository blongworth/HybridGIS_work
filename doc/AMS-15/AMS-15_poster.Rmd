...
```{r cur-flow-os, fig.height=4, dev='svg'}
ggplot(data, aes(co2flow, he12C)) +
  geom_smooth(span = .3, se=FALSE, color = "#00b7bd") +
  geom_point(size = 3, color = "#0069b1") +
  labs(title = "Ion current is stable for a range of CO2 flow",
       subtitle = "Current vs. CO2 flow during vial dillution",
...
ggplot(aes(fm_consensus, fm_diff, color = Name)) + 
  ...
  labs(title = "Agreement of HGIS with graphite measurements",
       ...
       x = "Fm expected (points jittered for clarity)", 
       y = "Fm difference (HGIS - expected)")
```
