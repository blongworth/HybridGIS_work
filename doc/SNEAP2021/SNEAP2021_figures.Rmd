...
df <- map_dfr(list.files(here("data_analysed"), full.names = TRUE), read_csv) %>% 
  ...
  mutate(fm_diff = mean - fm_consensus,
         fm_normdiff = normFm(mean, fm_consensus),
...
secs <- df %>% 
  filter(fm_diff < .1) %>% 
ggplot(aes(fm_consensus, fm_diff, color = Cur)) +
  geom_hline(yintercept = 0) +
  ...
  lims(x = c(0, 1.1),
       y = c(-.15, .15)) +
  ggtitle("Difference from consensus",
...
secs_lbc <- df %>% 
  filter(fm_diff < .1) %>% 
  mutate(mean_diff = mean(fmbc_diff)) %>% 
ggplot(aes(fm_consensus, fmbc_diff, color = Cur)) +
  geom_hline(yintercept = 0) +
  geom_smooth(method = "lm", se = FALSE) +
